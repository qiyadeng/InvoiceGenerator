...
public class PdfUtil {
    public static  void  createPDF(EinvoiceHead head, String templateFile, String desFile, String cachet, TaxRateDetailService detailService) throws IOException, DocumentException, WriterException {
        ...
        s.setField("buyername", head.getGmf_mc());//购买方名称
        s.setField("buyertaxno",head.getGmf_nsrsbh());
        s.setField("buyeraddtel",head.getGmf_dzdh());
        s.setField("buyerbank",head.getGmf_yhzh());
        s.setField("passwordarea", head.getFpmw());//发票密文

        EinvoiceBody  body = head.getEinvoiceBodies().get(0);
        TaxRateDetail detail = detailService.getByMergeCode(body.getSpbm());
        s.setField("product", "*"+detail.getProductName()+"*"+body.getXmmc());//项目名称
        ...
        s.setField("salebank", head.getXsf_dzdh());//销售方地址电话
        s.setField("saleaddtel", StringUtil.getValue(head.getXsf_yhzh()));//销售方银行账户
...
//        img.setAbsolutePosition(40,475);//控制图片位置
            ...
            img.setAbsolutePosition(40, 755);//控制图片位置

            PdfContentByte over = ps.getOverContent(1);
            over.addImage(img);
        }
        //画公章
        if(StringUtil.isNotEmpty(cachet)) {
            BufferedImage bufferedImage = ImageIO.read(new FileInputStream(cachet));
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            ...
            image.setAbsolutePosition(450,465);
            PdfContentByte over = ps.getOverContent(1);
            over.addImage(image);
        }
