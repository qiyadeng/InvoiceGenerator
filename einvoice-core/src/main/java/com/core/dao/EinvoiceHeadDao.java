...
public interface EinvoiceHeadDao extends CrudRepository<EinvoiceHead, Long> {
    ...
    //根据发票流水号查询查询
    @Query(nativeQuery = true,value = " select * from einvoice_head where dr=0 and fpqqlsh=:fpqqlsh")
    List<EinvoiceHead> getByFpqqlsh(@Param("fpqqlsh") String fpqqlsh);
    ...
    //根据发票状态和商户编码查找发票信息
    @Query(nativeQuery=true,value="select * from einvoice_head where dr=0 and status=:status and merchant_code=:merchantCode")
    List<EinvoiceHead> getByMerchantCodeAndStatus(@Param("merchantCode")String merchantCode,@Param("status") int status);
}
