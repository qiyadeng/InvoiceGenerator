...
public class EinvoiceHeadServiceImpl extends SuperServiceImpl<EinvoiceHeadMapper, EinvoiceHead> implements IEinvoiceHeadService {
    ...
    public void defaultDataGrid(PageInfo pageInfo){
        Page<EinvoiceHead> page = new Page<EinvoiceHead>(pageInfo.getNowpage(), pageInfo.getSize());
        List<EinvoiceHead> list = einvoiceHeadMapper.defaultHeadList(page, pageInfo.getCondition());
        pageInfo.setRows(list);
    }

    public EinvoiceHead selectEinvoiceHeadByFpqqlsh(String fpqqlsh){
        return einvoiceHeadMapper.selectEinvoiceHeadByFpqqlsh(fpqqlsh);
    }
}
