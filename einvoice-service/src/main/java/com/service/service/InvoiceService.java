...
public class InvoiceService {
     ...
     * 保存发票信息
     * @param order
     * @param changeCode 是否直接提交开票
     * @return
     */
    public InvoiceOrderResponse saveInvoiceInfo(InvoiceOrder order,boolean changeCode) {
        ...
        try {
            if(flag) {//验签通过
                ...
                if(einvoiceHead != null) {
                    String fpqqlsh = order.getFpqqlsh();
                    List<EinvoiceHead> einvoiceHeads = einvoiceService.getByFpqqlsh(fpqqlsh);
                    if(einvoiceHeads.size() > 0){
                        EinvoiceHead head = einvoiceHeads.get(0);
                        BeanUtils.copyProperties(einvoiceHead, head,"pk_einvoicehead");
                        einvoiceService.update(head);
                    } else {
                        if (changeCode) {
    ...
    private EinvoiceHead transform(InvoiceOrder order){
        ...
        einvoiceHead.setBmbbbh(order.getBmbbbh());//编码表版本号
        ...
        //dsy add
        einvoiceHead.setGmf_email(order.getGmf_email());
        einvoiceHead.setGmf_mobile(order.getGmf_phone());
        ...
        for(InvoiceOrderBody body:invoiceOrderBodies){
            ...
            einvoiceBody.setSpbm(order.getTax_spbm());//商品编码
